...
static int32_t SetResetStatus(TouchDriver *driver)
{
    ...
    writeBuf[0] = (GTP_REG_CMD_1 >> 8) & 0xFF; // 8:high byte 0xff：mask
    ...
    writeBuf[0] = (GTP_REG_CMD >> 8) & 0xFF; // 8:high byte 0xff：mask
...
static void EventHandle(TouchDriver *driver, ChipDevice *chipDev)
{
    ...
    if (ret == HDF_SUCCESS) {
        ...
        HDF_LOGE("%s: DataHandle failed", __func__);
    }
}
...
static int32_t ChipEsdResetRetry(TouchDriver *driver)
{
    ...
    HDF_LOGI("start ChipEsdResetRetry");
    ...
    HDF_LOGI("end ChipEsdResetRetry");
